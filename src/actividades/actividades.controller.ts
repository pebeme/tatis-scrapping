...
export class ActividadesController {
    ...
    async download(@Res() res: Response) {
        ...
        for (const city of cities) {
            ...
            let allActivityLinks = new Set<string>();  // Usamos un Set para evitar duplicados
            let allActivitiesData: any[] = [];

            // Función para obtener las URLs de las actividades de una página específica
            const getActivityLinksFromPage = async (pageUrl: string): Promise<string[]> => {
                const pageInstance = await browser.newPage();
                await pageInstance.goto(pageUrl, { waitUntil: 'networkidle2' });

                const activityLinks = await pageInstance.$$eval('a._activity-link', links =>
                    links.map(link => link.href)  // Obtener el href de cada enlace
                );
            ...
            const getActivityDetails = async (activityUrl: string) => {
                ...
                const name = await pageInstance.$eval('h1', h1 => h1.textContent.trim());

                // Extraer el precio (class a-text--price--big)
                let priceValue = 'No disponible';
                let priceCurrency = 'No disponible';
                try {
                    const priceText = await pageInstance.$eval('.a-text--price--big', span => span.textContent.trim());
                ...
                try {
                    const ratingText = await pageInstance.$eval('.a-text--rating-total', span => span.textContent.trim());
            ...
            for (let activityUrl of allActivityLinks) {
            ...
            xlsx.utils.book_append_sheet(workbook, worksheet, city.charAt(0).toUpperCase() + city.slice(1));  // Nombre de la hoja capitalizando la ciudad
        }
